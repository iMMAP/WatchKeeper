...
function sendTop6($ref_id, $msisdn,$message,$user_id,$group_id,$subgroup_id ="",$unique_id,$dbObj,$type)
{
	...
	if($xml->response == 'OK')
	   {
			...
			values ('".pg_escape_string($ref_id)."','".pg_escape_string($msisdn)."','".pg_escape_string($message)."','".pg_escape_string($group_id)."','".pg_escape_string($subgroup_id)."','".pg_escape_string($user_id)."','".pg_escape_string($unique_id)."',2,NOW(),'".pg_escape_string($msgId)."','$type')";
...
if($error)
 {
		$to  = 'mkhan@immap.org' ; 
				$subject = "Flash Alerts Karachi";
				$message = "Dear ,<br /><br />

							".$error."<br /><br />
							
							Flash Alerts Karachi";
				
				
					
				$headers  = "MIME-Version: 1.0\r\n";
				$headers .= "From: Flash Alerts Karachi <noreply@immap.org>\r\n";
					  ...
					  if($xmlsms->response == 'OK')
						{
							...
							$sqlQuery ="INSERT INTO sms_inbound (ref_id,msisdn_to,message,group_id,subgroup_id,user_id,unique_id,status,date_time,msgId,type) 
				values ('".pg_escape_string($ref_id)."','".pg_escape_string($msisdn)."','".pg_escape_string($text)."','".pg_escape_string($group_id)."','".pg_escape_string($subgroup_id)."','".pg_escape_string($user_id)."','".pg_escape_string($unique_id)."',2,NOW(),'".pg_escape_string($msgId)."','".pg_escape_string($type)."')";
				 ...
				 else
				  {
					   ...
					   $err = "";
						...
						$xml_ping = curl_exec($ch3);
						curl_close($ch3);
						$xml_ping = new SimpleXMLElement($xml_ping); 
